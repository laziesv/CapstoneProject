...
def _legacy_schema_already_exists() -> bool:
    existing = set(sa.inspect(op.get_bind()).get_table_names())
    present = existing & _CORE_TABLES
    if not present:
        return False
    missing = _CORE_TABLES - existing
    if missing:
        raise RuntimeError(
            'พบ schema เดิมไม่ครบ จึงหยุดก่อนสร้างตารางทับ: '
            + ', '.join(sorted(missing))
        )
    return True
...
def upgrade() -> None:
    """Upgrade schema."""
    # ฐานข้อมูลทีมมีตารางเดิมอยู่แล้ว ให้ merge migration ปรับแบบ additive
    if _legacy_schema_already_exists():
        return
...
def downgrade() -> None:
    """Downgrade schema."""
    # ห้ามลบตารางธุรกิจเดิมเมื่อย้อนเฉพาะสาย migration ใหม่
