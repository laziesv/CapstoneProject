...
class PersonalizedWatermarkExtractionTests(unittest.TestCase):
    ...
    def test_legacy_dynamic_hash_does_not_change_extracted_pixels(self):
        ...
        static = system.embed_static(
            original,
            evidence_uuid="synthetic-evidence",
        )
        personalized = system.embed_dynamic(static, self.ACCESS_SESSION_REF)

        without_hash = system.extract(personalized, original)
        repeated = system.extract(personalized, original)

        np.testing.assert_array_equal(without_hash[0], repeated[0])
        np.testing.assert_array_equal(without_hash[1], repeated[1])
